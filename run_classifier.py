...
def predict_protein_pairs(df, model_file):
    """
    Predict relative stability (Mesophile/Thermophile or More/Less Stable)
    for P1 and P2 proteins in each pair, based on model probabilities.

    Returns a table with:
    Protein | Prob_Thermo | Class
    """
    # === Load model ===
    ...
    # === Build feature matrices for P1 and P2 ===
    ...
    # === Determine pairwise stability based on probability ===
    p1_class, p2_class = [], []
    for prob1, prob2 in zip(p1_prob, p2_prob):
        if prob1 > prob2:
            p1_class.append("More Stable")
            p2_class.append("Less Stable")
        elif prob2 > prob1:
            p1_class.append("Less Stable")
            p2_class.append("More Stable")
        else:  # equal probability
            p1_class.append("Equal Stability")
            p2_class.append("Equal Stability")

    # === Build output table ===
    table = pd.DataFrame({
        "Protein": list(df["P1"]) + list(df["P2"]),
        "Prob_Thermo": list(p1_prob) + list(p2_prob),
        "Class": p1_class + p2_class
    })
