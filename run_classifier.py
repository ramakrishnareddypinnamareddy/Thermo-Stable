...
if __name__ == "__main__":
    ...
    # Check in CSV
    seq1_match, seq2_match = check_sequence_in_csv(seq1, seq2, CSV_FILE)

    # === Decision Logic ===
    # Thermo exact match
    if seq1_match == "Thermo" or seq2_match == "Thermo":
        thermo_seq = pdb1 if seq1_match == "Thermo" else pdb2
        print(f"Thermo_sequence — more thermostable.")
        sys.exit(0)

    # Meso exact match (only one)
    if (seq1_match == "Meso" and seq2_match != "Meso"):
        print(f" {pdb2} is more thermostable.")
        sys.exit(0)
    elif (seq2_match == "Meso" and seq1_match != "Meso"):
        print(f" {pdb1} is more thermostable.")
        sys.exit(0)

    # Both sequences found in same column or no match
    if (seq1_match == seq2_match) and seq1_match in ["Meso", "Thermo"]:
        print(f"Both sequences found similar")
    elif seq1_match is None and seq2_match is None:
        print("No sequence match found — proceeding with feature-based classification...")
    ...
    #electro_values = parse_foldx_output(foldx_out)
    ...
    protein_name=pdb1.split('_')[0]+'.csv'
    df = save_protein_features(data, protein_name)
    # Assuming you already have a dataframe `df_features` from save_protein_features
    df_pred = predict_protein_pairs(
    df=df,
